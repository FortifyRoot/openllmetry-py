"""
FR VCR cassette tests for safety rule enforcement on Anthropic instrumentation.

These tests record real Anthropic API interactions via VCR cassettes and verify
...
# ---------------------------------------------------------------------------
# T2-B3: RegEx rule — PII.email in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_regex_email_prompt(
...
# ---------------------------------------------------------------------------
# T2-B3: RegEx rule — PCI.credit_card in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_regex_credit_card_prompt(
    instrument_legacy, span_exporter, anthropic_client
):
...
# ---------------------------------------------------------------------------
# T2-B3: List rule — PII.competitor_org in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_list_competitor_org_prompt(
    instrument_legacy, span_exporter, anthropic_client
):
...
# ---------------------------------------------------------------------------
# T2-B3: UDF rule — custom_compliance with 2 sub-rules in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_udf_project_codenames_prompt(
    instrument_legacy, span_exporter, anthropic_client
):
...
# ---------------------------------------------------------------------------
# T2-B3: Completion safety — email in LLM response
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_completion_email_masked(
    instrument_legacy, span_exporter, anthropic_client
):
    """
...
# ---------------------------------------------------------------------------
# T2-B3: Combined — multiple rule types in single prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_combined_rules_prompt(
    instrument_legacy, span_exporter, anthropic_client
):
...
# ---------------------------------------------------------------------------
# T2-B3: Streaming with PII — verify safety on streamed completion
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_streaming_with_pii(
    instrument_legacy, span_exporter, anthropic_client
):
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_allow_email_not_masked(
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_allow_credit_card_not_masked(
    _set_allow_action, instrument_legacy, span_exporter, anthropic_client
):
