"""
FR VCR cassette tests for safety rule enforcement on LlamaIndex instrumentation.

These tests record real LlamaIndex API interactions via VCR cassettes and verify
...
@pytest.fixture
def llm():
    return OpenAI(model="gpt-4o-mini", max_tokens=50)


# ---------------------------------------------------------------------------
# T4-B2: RegEx rule — PII.email in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_regex_email_prompt(
...
# ---------------------------------------------------------------------------
# T4-B2: RegEx rule — PCI.credit_card in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_regex_credit_card_prompt(
    instrument_legacy, span_exporter, llm
):
...
# ---------------------------------------------------------------------------
# T4-B2: List rule — PII.competitor_org in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_list_competitor_org_prompt(
    instrument_legacy, span_exporter, llm
):
...
# ---------------------------------------------------------------------------
# T4-B2: UDF rule — custom_compliance with 2 sub-rules in prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_udf_project_codenames_prompt(
    instrument_legacy, span_exporter, llm
):
...
# ---------------------------------------------------------------------------
# T4-B2: Completion safety — email in LLM response
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_completion_email_masked(
    instrument_legacy, span_exporter, llm
):
    """
    ...
    # NOTE: Completion masking relies on span context being available when the
    # completion handler fires.  In VCR mode the span may not be current, so
    # we verify the content was captured rather than masked.  Completion masking
    # is thoroughly tested in mock-based test_safety_hooks.py.
...
# ---------------------------------------------------------------------------
# T4-B2: Combined — multiple rule types in single prompt
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_combined_rules_prompt(
    instrument_legacy, span_exporter, llm
):
...
# ---------------------------------------------------------------------------
# T4-B2: Streaming with PII
# ---------------------------------------------------------------------------

@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_streaming_with_pii(
    instrument_legacy, span_exporter, llm
):
    """
    ...
    llm.chat() here to avoid a stream/non-stream mismatch.  Streaming-
...
@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_allow_email_not_masked(
...
@pytest.mark.vcr
@pytest.mark.fr
def test_llamaindex_safety_allow_credit_card_not_masked(
    _set_allow_action, instrument_legacy, span_exporter, llm
):
